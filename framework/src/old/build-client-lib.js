...
const actionTemplate = Handlebars.compile(
  readFileSync(path.join(__dirname, "templates", "action.hbs"), "utf8")
);
const actionDeclarationTemplate = Handlebars.compile(
  readFileSync(
    path.join(__dirname, "templates", "action-declaration.hbs"),
    "utf8"
  )
);
  ...
  await Promise.all(
    actions.map(async actionName => {
      ...
      const resultInterfaceSrc = hasResult
        ? await j2t.compile(resultsSchema, "Result", { bannerComment: "" })
        : null;
